...
J_alpha_unc = 0.012;   % Inerzia di pitch [kg*m^2]
J_y_unc     = 0.00023; % Inerzia rotore [kg*m^2]
J_z_unc     = 0.00364; % Inerzia rotore [kg*m^2]
I_b_unc     = 0.00023; % Inerzia corpo base [kg*m^2]
m_unc       = 0.2;     % Massa dell'elicottero [kg]
l_unc       = 0.2;     % Distanza dal perno al baricentro [m]
c_alpha_unc = 0.01;    % Coefficiente di attrito viscoso pitch [N*m*s/rad]
c_beta_unc  = 0.01;    % Coefficiente di attrito viscoso yaw [N*m*s/rad]
eps_p_unc   = 0.1;     % Coefficiente di cross-thrust su pitch
eps_y_unc   = 0.1;     % Coefficiente di cross-thrust su yaw
g_unc       = 9.81;    % Accelerazione di gravità [m/s^2]
...
X0_PF = zeros(4, N_PF);
X0_PF(1, :) = sqrt(var_alpha_PF) * randn(1, N_PF); % Dispersione iniziale Alpha
X0_PF(2, :) = sqrt(var_dot_PF)   * randn(1, N_PF); % Dispersione iniziale Alpha_dot
X0_PF(3, :) = sqrt(var_beta_PF)  * randn(1, N_PF); % Dispersione iniziale Beta
X0_PF(4, :) = sqrt(var_dot_PF)   * randn(1, N_PF); % Dispersione iniziale Beta_dot
